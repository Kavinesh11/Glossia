...
// CWASA requires eval, so serve its page over HTTP rather than extension CSP.
const AVATAR_URL = 'http://localhost:5000/avatar/avatarnew.html';
    ...
    const checkWindowLoaded = setInterval(() => {
      ...
      try {
        if (avatarWindow.document.readyState === 'complete') {
          clearInterval(checkWindowLoaded);
          if (!avatarReady) updateAvatarStatus('Avatar: Loading...');
        }
      } catch (e) {
...
// Send text to the already-loaded avatar without reloading CWASA.
function sendTextToAvatar(text) {
  ...
  try {
    ...
    avatarWindow.postMessage({ type: 'PLAY_TEXT', text }, 'http://localhost:5000');
  } catch (error) {
...
window.addEventListener('message', (event) => {
  if (event.origin !== 'http://localhost:5000' || event.source !== avatarWindow) return;
  if (event.data?.type !== 'AVATAR_READY') return;
