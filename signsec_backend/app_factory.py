...
def create_app() -> Flask:
    ...
    avatar_root = Path(__file__).resolve().parent.parent / "extension" / "work"
    ...
    def avatar_remote_file(filename: str):
        response = requests.get(f"{avatar_upstream}{filename}", timeout=20)
    ...
    def avatar_file(filename: str):
        ...
        return send_from_directory(avatar_root, filename)
